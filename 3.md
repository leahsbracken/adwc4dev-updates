...
I  this lab will go through the steps to build a link between an ADW and an ATP in order to share data between the two databases. The steps consist of building a table in ADW, exporting that table to a BLOB (a binary large object), and then exporting that BLOB to the ATP.
